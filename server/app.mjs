...
export function createApp({ allowFraming = false } = {}) {
  ...
  // served from public/uploads so they remain available as public assets.
  ...
  publicApi.get("/products", async (req, res) => {
    res.json({
      products: await listProducts({
        isLoggedIn: Boolean(optionalCustomerId(req)),
      }),
    });
  });
  ...
  publicApi.patch(
    "/checkout/payment-intent/:pendingCheckoutId",
    writeLimiter,
    async (req, res) => {
      const result = await updatePaymentAmount(req.params.pendingCheckoutId, {
        ...req.body,
        customerId: optionalCustomerId(req),
      });
      res.status(result.status).json(result.body);
  ...
  publicApi.get(
    "/checkout/payment-intent/:pendingCheckoutId/status",
    readLimiter,
    async (req, res) => {
  ...
  publicApi.get("/drops", async (req, res) =>
    res.json({ drops: await listDropTeasers() }),
  );

  publicApi.get("/journal", async (req, res) =>
    res.json({ posts: await listPublishedPosts() }),
  );
  ...
  publicApi.get(
    "/gift-cards/purchase/:pendingCheckoutId/status",
    readLimiter,
    async (req, res) => {
      const result = await getGiftCardPaymentStatus(
        req.params.pendingCheckoutId,
      );
      res.status(result.status).json(result.body);
    },
  );
  ...
  publicApi.get("/content", async (req, res) =>
  ...
  accountRouter.post("/addresses", async (req, res) => {
    const result = await accountApi.createAddress(
      req.customer.sub,
      req.body || {},
    );
    res.status(result.status).json(result.body);
  ...
  accountRouter.put("/addresses/:id", async (req, res) => {
    const result = await accountApi.updateAddress(
      req.customer.sub,
      req.params.id,
      req.body || {},
    );
  ...
  accountRouter.delete("/addresses/:id", async (req, res) => {
    const result = await accountApi.deleteAddress(
      req.customer.sub,
      req.params.id,
    );
    res.status(result.status).json(result.body);
  ...
  accountRouter.post("/addresses/:id/default", async (req, res) => {
    const result = await accountApi.setDefaultAddress(
      req.customer.sub,
      req.params.id,
    );
    res.status(result.status).json(result.body);
  ...
  accountRouter.put("/preferences", async (req, res) => {
    const result = await accountApi.updatePreferences(
      req.customer.sub,
      req.body || {},
    );
    res.status(result.status).json(result.body);
  ...
  accountRouter.post("/wishlist", async (req, res) => {
    const result = await accountApi.addWishlistItem(
      req.customer.sub,
      (req.body || {}).productId,
    );
    res.status(result.status).json(result.body);
  ...
  accountRouter.delete("/wishlist/:productId", async (req, res) => {
    const result = await accountApi.removeWishlistItem(
      req.customer.sub,
  ...
  accountRouter.post("/wishlist/merge", async (req, res) => {
    const result = await accountApi.mergeWishlist(
      req.customer.sub,
      (req.body || {}).productIds || [],
    );
    res.status(result.status).json(result.body);
  ...
  accountRouter.get("/returns", async (req, res) => {
    res.json({
      returnRequests: await listMyReturnRequests(req.customer.email),
    });
  });
  ...
  adminApi.get("/customers", async (req, res) =>
    res.json({ customers: await listCustomers() }),
  );

  adminApi.get("/newsletter", async (req, res) =>
  ...
  adminApi.get("/overview", async (req, res) => {
    const [
      ...
      get("SELECT COUNT(*) c FROM products WHERE archived = 0").then(
        (r) => r.c,
      ),
      get("SELECT COUNT(*) c FROM orders").then((r) => r.c),
      get("SELECT COUNT(*) c FROM reviews WHERE status = 'published'").then(
        (r) => r.c,
      ),
      get(
        "SELECT COALESCE(SUM(total), 0) r FROM orders WHERE status != 'cancelled'",
      ).then((r) => Number(r.r)),
      get(
        "SELECT COUNT(*) c FROM orders WHERE status = 'payment_pending'",
      ).then((r) => r.c),
  ...
  adminApi.get("/products", async (req, res) =>
    res.json({ products: await listProducts({ includeArchived: true }) }),
  );
  ...
  adminApi.get("/returns", async (req, res) =>
    res.json({ returnRequests: await listAllReturnRequests() }),
  );
  ...
  adminApi.get("/journal", async (req, res) =>
    res.json({ posts: await listAllPosts() }),
  );
  ...
  adminApi.get("/reports/summary", async (req, res) =>
    res.json(await getSummary()),
  );
  adminApi.get("/reports/revenue", async (req, res) =>
    res.json({ series: await getRevenueOverTime(req.query.days) }),
  );
  adminApi.get("/reports/top-products", async (req, res) =>
    res.json({ products: await getTopProducts(req.query.limit) }),
  );
  adminApi.get("/reports/sales-by-category", async (req, res) =>
    res.json({ categories: await getSalesByCategory() }),
  );
  adminApi.get("/reports/order-status", async (req, res) =>
    res.json({ statuses: await getOrderStatusBreakdown() }),
  );

  adminApi.get("/reviews", async (req, res) =>
  ...
  adminApi.get("/editions", async (req, res) =>
    res.json({ editions: await listAllEditionSales() }),
  );
  ...
  adminApi.get("/promo-codes", async (req, res) =>
    res.json({ promoCodes: await listPromoCodes() }),
  );
  ...
  adminApi.get("/gift-cards", async (req, res) =>
  ...
  adminApi.put("/gift-cards/:id/active", async (req, res) => {
    const result = await setGiftCardActive(
      req.params.id,
  ...
  adminApi.get("/content", async (req, res) =>
    res.json({ content: await getSiteContent() }),
  );
  ...
  adminApi.put("/content/:section", async (req, res) => {
    const result = await updateSiteContentSection(
      req.params.section,
      req.body || {},
    );
    res.status(result.status).json(result.body);
  ...
  adminApi.post("/uploads", (req, res) => {
    uploadImage(req, res, async (err) => {
      if (err) {
        res
          .status(400)
          .json({ error: err.message || "Could not upload file." });
        return;
      }
      if (!req.file) {
      ...
      if (!verifyUploadedFileContent(req.file.path, ALLOWED_IMAGE_TYPES)) {
        ...
        fs.unlink(req.file.path, () => {
          res
            .status(400)
            .json({ error: "This file does not appear to be a valid image." });
        });
        return;
      }
      ...
      try {
        const dimensions =
          readImageDimensions(req.file.path, req.file.mimetype) || {};
  ...
  adminApi.post("/uploads/video", (req, res) => {
    uploadVideo(req, res, async (err) => {
      if (err) {
        res
          .status(400)
          .json({ error: err.message || "Could not upload video." });
        return;
      }
      if (!req.file) {
        res.status(400).json({ error: "No file was uploaded." });
      ...
      if (!verifyUploadedFileContent(req.file.path, ALLOWED_VIDEO_TYPES)) {
        ...
        fs.unlink(req.file.path, () => {
          res
            .status(400)
            .json({ error: "This file does not appear to be a valid video." });
        });
        return;
      }
      const url = `/uploads/${req.file.filename}`;
  ...
  app.get("/sitemap.xml", async (req, res) => {
    ...
    const productPaths = (await listProducts({ isLoggedIn: false })).map(
      (p) => `/product/${encodeURIComponent(p.id)}`,
    );
