...
// Called once, right after a file is written to data/uploads/ by the
...
export async function recordMedia({ filename, url, mimeType, kind, width, height, sizeBytes }) {
  const id = randomUUID();
  await run(
    'INSERT INTO media (id, filename, url, mimeType, kind, width, height, sizeBytes, alt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
    [id, clean(filename, 255), clean(url, 500), clean(mimeType, 100), kind === 'video' ? 'video' : 'image', width ?? null, height ?? null, Number(sizeBytes) || 0, '']
  );
...
export async function updateMediaAlt(id, alt) {
  ...
  await run('UPDATE media SET alt = ? WHERE id = ?', [clean(alt, 300), mediaId]);
  return { status: 200, body: { media: serialize(await get('SELECT * FROM media WHERE id = ?', [mediaId])) } };
}
...
function containsUrl(value, url) {
  ...
  if (value && typeof value === 'object') return Object.values(value).some((v) => containsUrl(v, url));
  return false;
}
...
export async function findMediaUsage(url) {
  ...
  for (const row of contentRows) {
    ...
    if (containsUrl(data, url)) locations.push(SITE_CONTENT_LABELS[row.section] || `Site Content → ${row.section}`);
  }

  const productRows = await all('SELECT id, name, images, garmentExplorer FROM products WHERE archived = 0');
  for (const row of productRows) {
    const images = fromJson(row.images, []);
    const explorer = fromJson(row.garmentExplorer, null);
    if (Array.isArray(images) && images.includes(url)) locations.push(`Product → ${row.name} (gallery)`);
    if (explorer && containsUrl(explorer, url)) locations.push(`Product → ${row.name} (Garment Explorer)`);
  }

  const journalRows = await all('SELECT id, title, coverImage FROM journal_posts');
...
export async function deleteMedia(id, { force = false } = {}) {
  ...
  if (usage.length && !force) {
    return { status: 409, body: { error: 'This file is currently in use and was not deleted.', usage } };
  }
  ...
  } catch { /* non-fatal — DB is the source of truth for the Media Library listing */ }
