...
// it's always the real project's data/uploads/ directory, unlike every
...
test('a real PNG (correct magic bytes) is accepted', async () => {
  ...
  assert.ok(existsSync(path.join(uploadsDir, result.body.url.replace('/uploads/', ''))), 'the accepted file should actually exist on disk');
});
...
test('SPOOFED upload: a file claiming image/jpeg but actually containing an HTML/script payload is rejected, not saved', async () => {
  const maliciousContent = Buffer.from('<script>alert(document.cookie)</script>', 'utf8');
...
test('SPOOFED upload: a file claiming image/png but actually plain text is rejected', async () => {
  const result = await uploadFile(Buffer.from('just some plain text, not an image'), 'notanimage.png', 'image/png');
  assert.equal(result.status, 400);
});
...
test('a rejected spoofed upload does not leave a file behind on disk', async () => {
  ...
  assert.equal(newFiles.length, 0, `a rejected upload must be deleted, not left on disk — found new file(s): ${JSON.stringify(newFiles)}`);
});
